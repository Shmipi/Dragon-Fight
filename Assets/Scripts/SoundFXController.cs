...
public static class SoundFXController
{
    ...
    private static void Initialize() // used to initialize Dictionary. Should be called in "Awake" when game starts
    {
    ...
    public static void PlaySound(Sound sound) //choses wich sound should be played
    {
        if (CanPlaySound(sound))
        {
            ...
            AudioSource audioSource = soundGameObject.AddComponent<AudioSource>(); //this komponent plays the Audio
            audioSource.PlayOneShot(GetAudioClip(sound));  // funktion that choses how the sound should be played
        }
    }
